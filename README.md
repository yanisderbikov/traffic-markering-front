...
суммы криатор может забирать заработанное) и площадки, с которых принимает ролики
(YouTube Shorts, TikTok, Instagram — в редакторе есть «выбрать все» и «убрать все»).
Порог виден на карточке и странице объявления и в откликах криатора. **Криатор** берёт
...
одного криатора и период приёма откликов (даты по Москве, включительно; вне периода
объявление уходит с доски, а страница отклика закрыта). Требования видны на карточке
доски, на странице объявления и над формой отклика — до того, как криатор что-то
...
ссылки — бриф, баннеры, референсы; криатор открывает или скачивает их со страницы
...
если криатор подключил канал с доступом к аналитике; TikTok и Instagram её не отдают,
поэтому при регионе «только РФ» или «СНГ» ролики с них не оплачиваются вовсе. Редактор
предупреждает об этом заказчика, когда такие площадки выбраны вместе с регионом; криатор
...
Криатор видит заработок на `/app/earnings`: начисления за просмотры приходят в кошелёк
...
прикладывает скриншоты и номер транзакции, а криатор на странице операции
...
| `/app/finance/payouts` | `FinancePayouts` — заявки криаторов на выплату с фильтром по статусу | FINANCE_MANAGER, SUPER_ADMIN |
...
`ADMIN` видит секции заказчика и криатора, `SUPER_ADMIN` — их же плюс финансы и
