...
const FinanceCustomer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await apiClient.api.withdrawFromWallet(Number(userId), {
        amountKopecks,
        txId: txId.trim(),
      ...
      setComment('');
      toast.success(
        `С кошелька выведено ${formatRubles(amountKopecks)} — ждём подтверждения рекламодателя`
      );
  ...
  return (
    <div className={ui.page}>
      ...
      <form className={`${ui.card} ${styles.block}`} onSubmit={handleSubmit} noValidate>
        ...
        <p className={ui.hint}>
          Пополнения рекламодатель заводит сам — они в разделе{' '}
          <Link to="/app/finance/top-ups" className={ui.linkAccent}>
            «Пополнения»
          </Link>
          .
        </p>
